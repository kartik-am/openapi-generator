...
SingleRefType::SingleRefType(boost::property_tree::ptree const& pt)
{
        fromPropertyTree(pt);
}
...
void SingleRefType::fromJsonString(std::string const& jsonString)
{
...
ptree SingleRefType::toPropertyTree() const
{
	ptree pt;
	ptree tmp_node;
	return pt;
...
void SingleRefType::fromPropertyTree(ptree const &pt)
{
	ptree tmp_node;
}
...
std::vector<SingleRefType> createSingleRefTypeVectorFromJsonString(const std::string& json)
{
    ...
    auto vec = std::vector<SingleRefType>();
    for (const auto& child: pt) {
        vec.emplace_back(SingleRefType(child.second));
    }
